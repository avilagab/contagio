...
const params = {
	minCuradores: N * 0.05,
	maxCuradores: N * 0.25,
	maxCuradoresLimit: N * 0.75,
	propagationInterval: 1500,			//750
	propagationProbability: 0.3, 		//0.25
	cureProbability: 0.2,				//0.125
...
gui.add(params, 'propagationProbability', 0, 1).step(0.01).name('Probabilidad Contagio');
gui.add(params, 'cureProbability', 0, 1).step(0.01).name('Probabilidad Cura');
...
let curadores = [];
//const minCuradores = N*.05;  // mínimo valor posible
//let maxCuradores = N*.25; // límite máximo
//const maxCuradoresLimit = N*.50;; // máximo valor posible
//const propagationInterval = 1500; // ms, propagación cada medio segundo
//const propagationProbability = 0.25; // 20% probabilidad de contagio por vecino cada frame
//const cureProbability = 0.125; // 20% probabilidad de curar a vecino cada iteración

const curadoresChangeInterval = 5000;  // tiempo en ms para cambiar valor
let lastChangeTime = 0;

const infectionInterval = 300; // ms, cada 5 segundos
...
// Crear conexiones aleatorias: cada nodo tendrá de 8 a 11 conexiones
for (let i = 0; i < N; i++) {
	const numConexiones = Math.floor(Math.random() * N *.01) + 2; // 8 a 11 conexiones
...
//Propagar un mensaje adecuado a modo de cura
function propagateCure() {
  let newCures = [];
	...
	newCures.forEach(node => {
		node.infected = false;
		node.mesh.material.color.copy(node.baseColor); // restaurar color base
	});

	// Limitar crecimiento para no saturar de curadores
	if (curadores.length < params.maxCuradores) {
		const spaceLeft = params.maxCuradores - curadores.length;
		curadores = curadores.concat(newCures.slice(0, spaceLeft));
	}
}
